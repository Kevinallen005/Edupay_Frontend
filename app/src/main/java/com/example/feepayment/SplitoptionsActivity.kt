...
import com.example.feepayment.adapters.SplitoptionsAdapter
...
class SplitoptionsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        setContentView(R.layout.activity_splitoptions)
        ...
        recyclerView = findViewById(R.id.splitrecycle)
    ...
    private fun fetchDueList(studentId: Int) {
        retrofit.instance.getfullonlyduefeeslist(studentId).enqueue(object : Callback<DueListResponse> {
            override fun onResponse(call: Call<DueListResponse>, response: Response<DueListResponse>) {
                if (response.isSuccessful && response.body()?.status == "success") {
                    ...
                    val adapter = SplitoptionsAdapter(this@SplitoptionsActivity, list) {
                        val sharedPref = getSharedPreferences("MyAppPrefs", Context.MODE_PRIVATE)
                        with(sharedPref.edit()) {
                            putString("feename", it.feename)
                            apply()
                        }
                        val intent = Intent(this@SplitoptionsActivity, SplitmainpageActivity::class.java)
                    ...
                    Toast.makeText(this@SplitoptionsActivity, "No Current EMI Options found", Toast.LENGTH_SHORT).show()
                }
            }

            override fun onFailure(call: Call<DueListResponse>, t: Throwable) {
                Toast.makeText(this@SplitoptionsActivity, "Error: ${t.message}", Toast.LENGTH_SHORT).show()
            }
        })
    }
}
